...
echo "--- Starting migration e2e tests for Odoo version $ODOO_VERSION ---"
...
START_TIME=$(date +%s)
until docker compose logs odoo | grep -q "HTTP service (werkzeug) running"; do
  CURRENT_TIME=$(date +%s)
...
docker compose exec -T odoo odoo -d odoo_data_flow_source_db -i base,contacts --without-demo=True --stop-after-init
...
docker compose exec -T odoo odoo -d odoo_data_flow_target_db -i base,contacts --without-demo=True --stop-after-init
...
# 5. Run odoo-data-flow migrate
echo "--- Running migration... ---"
./.venv/bin/python3 -m odoo_data_flow migrate \
    --config-export conf/source.conf \
    --config-import conf/target.conf \
    --model res.partner \
...
docker compose exec -T odoo python3 /odoo-data-flow/tests/e2e/verify_migration_data.py odoo_data_flow_target_db

# Cleanup containers
docker compose down -v

echo "--- Migration e2e tests completed for Odoo version $ODOO_VERSION ---"
