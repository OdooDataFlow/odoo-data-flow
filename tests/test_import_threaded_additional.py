...
def test_convert_external_id_field_empty() -> None:
    ...
    base_name, converted_value = _convert_external_id_field(
        mock_model, "parent_id/id", ""
    )
    assert base_name == "parent_id"
    assert not converted_value  # Empty value should return False
...
def test_convert_external_id_field_exception() -> None:
    ...
    base_name, converted_value = _convert_external_id_field(
        mock_model, "parent_id/id", "some_ref"
    )
    assert base_name == "parent_id"
...
def test_setup_fail_file_with_error_reason_column() -> None:
    ...
    with patch("builtins.open") as mock_open:
        ...
        if writer:
            # writer.writerow should be called with original
            # headers since _ERROR_REASON already exists
